...
export function SmartcapMetricDemo() {
  ...
  return (
    <ExperienceShell
      ...
      hint={[
        'three.js 실제 3D 장면 — 슬라이더로 움직인 객체를 카메라로 투영한 2D 값(박스 높이·단면 지름·소실점 y)에 실제 저장소의 판별식을 그대로 적용합니다.',
        '임계값도 실제 값 그대로: 차량 +50%/+200% · 자재 단면 ×1.1/×1.35 · 계단 소실점 기준선(밑변 y≥634).',
        '체험은 조작감을 위해 등급이 실시간으로 오르내리는 양방향 — 실제 시스템은 단방향 상승 + 미감지 시 리셋입니다.',
      ]}
    >
