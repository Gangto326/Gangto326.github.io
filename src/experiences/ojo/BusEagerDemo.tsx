  ...
  return (
    ...
    >
      ...
      <div ref={seqRef} style={{ scrollMarginTop: 80 }} className="mt-4">
        ...
        >
          <ChevronDown className={`h-4 w-4 transition-transform ${showSeq ? 'rotate-180' : ''}`} />
          실제 시퀀스 다이어그램 {showSeq ? '접기' : '보기'}
        </button>
        {showSeq && (
          <div className="mt-3 overflow-auto rounded-xl border border-black/10 bg-white p-3">
            <img src={SEQ} alt="버스 번호 인식 Pre-fetching 시퀀스 다이어그램" className="w-full min-w-[560px]" />
          </div>
        )}
      </div>
    </ExperienceShell>
